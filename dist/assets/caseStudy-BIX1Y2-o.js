import{t as e}from"./dist-CcLWAJ7j.js";/* empty css              */import{n as t,r as n,t as r}from"./lenis-B1cd2kmr.js";var i=e(`https://nojgcsocmocwegfxtxse.supabase.co`,`sb_publishable_jYg3PjN-4vZJlaOvWHIr1w_dh8ubWlD`),a=new r({duration:1.2,easing:e=>Math.min(1,1.001-2**(-10*e)),direction:`vertical`,gestureDirection:`vertical`,smooth:!0,mouseMultiplier:1,smoothTouch:!1,touchMultiplier:2,infinite:!1});function o(e){a.raf(e),requestAnimationFrame(o)}requestAnimationFrame(o),n.registerPlugin(t);async function s(e){try{let{data:t,error:n}=await i.from(`projects`).select(`*`).eq(`slug`,e).eq(`published`,!0).single();if(n)throw n;return t}catch(e){return console.error(`Error fetching project:`,e),null}}function c(e){let t=document.getElementById(`project-container`);if(!e){t.innerHTML=`
  ...
  `,t.classList.remove(`hidden`),l()}function l(){n.timeline({defaults:{ease:`power4.out`,duration:1.2}}).fromTo(`.hero-anim`,{y:30,opacity:0},{y:0,opacity:1,delay:.2}).fromTo(`.hero-title`,{y:50,opacity:0,scale:.98},{y:0,opacity:1,scale:1},`-=0.8`).fromTo(`.hero-motto`,{y:30,opacity:0},{y:0,opacity:1},`-=1.0`),n.utils.toArray(`.story-block`).forEach(e=>{n.fromTo(e,{y:50,opacity:0},{y:0,opacity:1,duration:1,ease:`power3.out`,scrollTrigger:{trigger:e,start:`top 85%`,toggleActions:`play none none reverse`}})});let e=document.getElementById(`sticky-metrics`);e&&n.fromTo(e,{x:50,opacity:0},{x:0,opacity:1,duration:1,ease:`power3.out`,scrollTrigger:{trigger:e,start:`top 80%`,toggleActions:`play none none reverse`}}),n.utils.toArray(`.gallery-item img`).forEach(e=>{n.fromTo(e,{scale:.9,opacity:0,y:50},{scale:1,opacity:1,y:0,duration:1.5,ease:`power3.out`,scrollTrigger:{trigger:e,start:`top 90%`,end:`top 30%`,scrub:.5}})})}document.addEventListener(`DOMContentLoaded`,async()=>{let e=new URLSearchParams(window.location.search).get(`p`);if(!e){window.location.href=`/`;return}let t=document.getElementById(`loader`),n=await s(e);t.style.display=`none`,c(n)});