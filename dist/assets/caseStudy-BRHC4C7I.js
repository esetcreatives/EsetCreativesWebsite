import{n as e,t}from"./dist-SjEwtgV2.js";/* empty css              */import{n,r,t as i}from"./lenis-B1cd2kmr.js";e();var a=t(`https://nojgcsocmocwegfxtxse.supabase.co`,`sb_publishable_jYg3PjN-4vZJlaOvWHIr1w_dh8ubWlD`),o=new i({duration:1.2,easing:e=>Math.min(1,1.001-2**(-10*e)),direction:`vertical`,gestureDirection:`vertical`,smooth:!0,mouseMultiplier:1,smoothTouch:!1,touchMultiplier:2,infinite:!1});function s(e){o.raf(e),requestAnimationFrame(s)}requestAnimationFrame(s),r.registerPlugin(n);async function c(e){try{let{data:t,error:n}=await a.from(`projects`).select(`*`).eq(`slug`,e).eq(`published`,!0).single();if(n)throw n;return t}catch(e){return console.error(`Error fetching project:`,e),null}}function l(e){let t=document.getElementById(`project-container`);if(!e){t.innerHTML=`
  ...
  `,t.classList.remove(`hidden`),u()}function u(){r.timeline({defaults:{ease:`power4.out`,duration:1.2}}).fromTo(`.hero-anim`,{y:30,opacity:0},{y:0,opacity:1,delay:.2}).fromTo(`.hero-title`,{y:50,opacity:0,scale:.98},{y:0,opacity:1,scale:1},`-=0.8`).fromTo(`.hero-motto`,{y:30,opacity:0},{y:0,opacity:1},`-=1.0`),r.utils.toArray(`.story-block`).forEach(e=>{r.fromTo(e,{y:50,opacity:0},{y:0,opacity:1,duration:1,ease:`power3.out`,scrollTrigger:{trigger:e,start:`top 85%`,toggleActions:`play none none reverse`}})});let e=document.getElementById(`sticky-metrics`);e&&r.fromTo(e,{x:50,opacity:0},{x:0,opacity:1,duration:1,ease:`power3.out`,scrollTrigger:{trigger:e,start:`top 80%`,toggleActions:`play none none reverse`}}),r.utils.toArray(`.gallery-item img`).forEach(e=>{r.fromTo(e,{scale:.9,opacity:0,y:50},{scale:1,opacity:1,y:0,duration:1.5,ease:`power3.out`,scrollTrigger:{trigger:e,start:`top 90%`,end:`top 30%`,scrub:.5}})})}document.addEventListener(`DOMContentLoaded`,async()=>{let e=new URLSearchParams(window.location.search).get(`p`);if(!e){window.location.href=`/`;return}let t=document.getElementById(`loader`),n=await c(e);t.style.display=`none`,l(n)});